...
import { checkPermissions }      from '../utils/permissionGuard.js';
...
export async function handleShortcut(message, client) {
    ...
    // ── Permission check ──────────────────────────────────────────────────────
    try {
        const permResult = await checkPermissions(interaction, schema.command);
        if (!permResult.allowed) {
            return message.reply({ embeds: [permResult.embed] });
        }
    } catch (err) {
        logger.error('[Shortcuts] Permission check failed:', err);
    }
